...
* This project uses their language model (`meta-llama/Meta-Llama-3.1-8B-Instruct`) to generate responses to a variety of Image questions, showcasing the power of their tools for developers.
...
2. **Clone the Repository**
   ```bash
   git clone https://github.com/CryptoAirdropHindi/Imagebot.app.git
   cd Imagebot.app
   ```
...
Replace the `YOUR_API_KEY_HERE` in `Imagebot.py` with your own [Hyperbolic API Key](https://app.hyperbolic.xyz/settings):
   ```bash
nano Imagebot.Py
   ```
5. **Run the Chatbot**
...
Execute the script to start the chatbot:
   ...
   python3 Imagebot.Py
   ```
* You can enter `screen -S Image` before running it to run the script on a minimizable screen in background.
* To minimize screen: `CTRL+A+D`
* To kill screen: `Ctrl+C` or command: `screen -XS Image quit`

## Usage
Once running, the chatbot will:
* Print each of the 100 Image questions one by one.
* Fetch and display answers from the Hyperbolic Labs API.
* Pause for a random interval (60-120 seconds) between Image questions.
* Stop after completing all 100 Image questions.
...
* Feel free to modify the `questions` list in `Imagebot.py` to suit your interests!
